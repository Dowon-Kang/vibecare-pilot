...
| 버전 | 용도 | 상태 |
|---|---|---|
| `pilot-0.6.0` | 현재 앱·서버·웹의 Mock 계산 | 연결됨 |
| `pilot-0.7.0` | ASM/SMM 정의를 분리한 네 건 평가 | 연구 모듈 |
| `adaptive-research-0.2.0` | 이력·RPE·세 체감에 따른 단계 후보 | 연구 모듈 |
| `research-gate-0.8.1` | 정확한 프로토콜·물리량·교정 검토 | 연구 모듈 |

최신 연구 설계는 [근거·수식·검증 문서](docs/algorithm/adaptive-research-v2.md)를 따른다. 나이·성별은 검증된 근육 기준 선택에만 사용하고, 체지방은 데이터 일관성과 향후 분석 변수로 사용한다. 근거 없는 0.90/0.95 곱셈이나 체지방 증가에 따른 자동 증량은 실장비 규칙으로 사용하지 않는다.
...
### Phase 2 — 연구 알고리즘 Mock 통합

구현 대상:

- 서버에서 네 측정으로 근육평가를 다시 계산
- 신뢰된 현재 단계·완전 이력·버전 정책 조회
- `evaluateAdaptiveResearch`를 별도 연구 미리보기 API에 연결
- 입력·평균·분류·피드백·선택 단계·물리량·근거 수준 표시
