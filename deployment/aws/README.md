...
2. 저장소의 공급자 중립 PostgreSQL 어댑터는 Supabase와 로컬 PostgreSQL에 우선 사용한다. 향후 AWS 담당자가 RDS를 선택해도 `backend-api/src/storage/postgres-database.ts`와 `deployment/local/postgres/001_schema.sql`을 재사용할 수 있다.
...
- DB: PostgreSQL 어댑터가 D1 형식 `?` placeholder를 `$n`으로 변환하고 `first/all/run/batch`를 제공한다. `batch`는 단일 연결의 트랜잭션으로 원자성을 보장한다. 통합 스키마는 기존 TEXT ISO 시각과 0/1 안전 플래그를 유지해 Route 호환성을 우선한다.
...
- Node HTTP 진입점과 graceful shutdown: `backend-api/src/node-server.ts`
- PostgreSQL 포트 어댑터와 원자적 batch: `backend-api/src/storage/postgres-database.ts`
- 최종 스키마와 장치 중복 세션 방지 trigger: `deployment/local/postgres/001_schema.sql`
- 로컬 PostgreSQL Compose와 합성 개발 seed

## 현재 차단사항

- 현재 앱 저장소는 Supabase PostgreSQL을 우선 대상으로 선택했다. ECS Fargate + ALB 및 RDS PostgreSQL은 후속 AWS 담당자가 필요 시 같은 PostgreSQL adapter를 재사용하는 인계안이며, 실제 AWS 리전·VPC·보안그룹·RDS는 생성되지 않았다.
- Docker Desktop 엔진이 실행되지 않는 PC에서는 로컬 PostgreSQL 종단간 검증을 수행할 수 없다.
