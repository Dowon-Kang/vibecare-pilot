...
2. `backend-api/src/app-context.ts`의 `SqlDatabase` 포트에 AWS 데이터베이스 어댑터를 연결한다. 운영 기본 후보는 RDS PostgreSQL이다. 현재 D1 SQL을 그대로 운영 SQL이라고 가정하지 않는다.
...
- DB: 어댑터는 파라미터 바인딩과 `first/all/run`을 제공하고, `batch`는 전체 성공 또는 전체 롤백의 원자성을 보장해야 한다. 현재 `ON CONFLICT`, `MIN/MAX`, `?` placeholder, SQLite 날짜·BOOLEAN 표현은 PostgreSQL 변환 대상이다.
...
- 애플리케이션은 DB 포트로 분리됐지만 AWS DB 어댑터가 없으므로 그대로 AWS에 실행할 수 없다.
