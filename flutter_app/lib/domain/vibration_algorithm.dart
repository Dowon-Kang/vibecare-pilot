...
double? _optionalMean(Iterable<double?> values) {
  ...
  if (list.length != requiredMeasurementCount ||
      list.any((value) => value == null)) {
    return null;
  }
  return _round(_mean(list.cast<double>()), 2);
}

bool _hasValidCoreValues(BiaMeasurement measurement) => measurement
    .values
    .requiredValues
...
AlgorithmResult calculateRecommendation({
  ...
  final safetyWarnings = <String>[
    if (safety.acutePain) '현재 통증이 있어 실행을 차단합니다.',
    if (safety.dizziness) '어지럼 증상이 있어 실행을 차단합니다.',
    if (safety.clinicianHold) '전문가 사용 보류 지시가 있습니다.',
  ];
