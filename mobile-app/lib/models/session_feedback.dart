...
class FeedbackAdjustment {
  ...
  FeedbackAdjustment next(SessionFeedback f, int used) {
    if (f.rpe < 0 || f.rpe > 10 || f.pain < 0 || f.pain > 10) {
      throw ArgumentError('설문 범위 오류');
    }
    final reduce = f.rpe >= 7 || f.intensityRating == FeedbackRating.strong;
    final candidate = reduce ? (used * .9).floor() : used;
    ...
    final hold =
        ...
        f.dizziness ||
        f.earlyStopped ||
        f.durationRating == FeedbackRating.strong ||
        f.frequencyRating == FeedbackRating.strong;
  ...
  AlgorithmResult apply(AlgorithmResult result, double minimum) {
    ...
    final intensity = intensityCap == null || rec.intensityPct < intensityCap!
        ? rec.intensityPct
        : intensityCap!;
