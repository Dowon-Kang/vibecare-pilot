...
const require = createRequire(pathToFileURL(root + 'services/api/package.json'));
const ts = require('typescript');
const urls = {};
for (const name of ['auth', 'algorithm', 'fitrus-client', 'index']) {
  let source = fs.readFileSync(root + `services/api/src/${name}.ts`, 'utf8');
  if (name === 'index') {
...
for (const file of ['0001_initial.sql', '0002_measurements_and_rules.sql']) db.exec(fs.readFileSync(root + 'services/api/migrations/' + file, 'utf8'));
class Statement {
