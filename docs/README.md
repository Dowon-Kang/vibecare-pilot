...
현재 연결된 규칙은 `pilot-0.8.0` 한 가지입니다. 학습된 AI 모델은 없으며, 부위 기준값과 명시적 계수를 사용하는 재현 가능한 규칙 기반 시뮬레이터입니다.
...
4. [pilot-0.8.0 알고리즘 설계](algorithm/pilot-0.8.0-design.md): 현재 구현의 수식, 안전 경계와 추적성
...
`pilot-0.8.0`은 연구용 시뮬레이션입니다. 테스트 통과는 코드 재현성을 뜻하며 임상 효과나 실장비 안전을 증명하지 않습니다.
