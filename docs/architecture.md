...
flowchart TD
    ...
    O --> P[전체 느낌·이상 반응 피드백<br/>필요 시 세부 조정]
```
...
Flutter 화면은 로그인 후 `프로필·주의사항 → 골격근량 결과·직전 기록 비교 → 인체 부위·설정 확인`의 로컬 표시 단계로 이동한다. 표시 단계는 서버 상태를 변경하지 않는다. 마지막 단계에서만 기존 `DeviceGateway.authorize`를 호출한다. 골격근 연구 등급은 최신 DB 이력의 SMM과 참여자 키로 표시용 계산하며, `pilot-0.8.0` 추천과 장치 명령은 계속 서버·공통 알고리즘 결과를 정본으로 사용한다.
