...
이 문서는 구현 기준을 요약한다. 상세 유스케이스와 상태도는 `architecture.md`, 공개 HTTP 계약은 `../shared-contracts/openapi.yaml`을 단일 기준으로 사용한다. 기존 React PWA는 비교·검증용이며 주 클라이언트는 Flutter다.
...
flowchart LR
    ...
    APP -->|PIN·Bearer token| API[백엔드 API]
    API -->|x-api-key, 서버 전용| FITRUS[FITRUS 측정 API]
    FITRUS --> API
    API --> DB[(데이터 저장소: 현재 D1·AWS 전환 예정)]
    DB --> API --> APP
    APP --> LOCAL[로컬 미리보기]
...
| 백엔드 API | PIN 잠금, 본인 데이터 권한, FITRUS 프록시, 최신 4건, authoritative 계산, 1회성 허가, 세션 로그 | 공급사 미확인 필드 임의 해석 |
| 데이터 저장소 | 원본 응답, canonical 측정, 규칙 버전, 추천, 명령·피드백 감사 이력. 현재 D1 형식이며 AWS DB 어댑터 필요 | PIN 원문과 공급사 API 키 저장 |
...
6. Flutter 계산은 미리보기다. 실제 실행 허가는 백엔드 API가 저장된 원본과 현재 규칙으로 다시 계산한다.
...
- 구현됨: Mock 로그인·데이터 repository·최근 4건·전체 값 화면·보정식·Mock 실행·중지, 백엔드 인증·조회·추천·Mock 세션 API, 현재 D1 확장 스키마.
- 검증 대기: Flutter analyze/test/APK, AWS 데이터 저장소 어댑터, 앱과 백엔드의 실제 HTTP 연결.
