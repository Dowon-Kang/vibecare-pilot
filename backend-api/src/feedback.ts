...
export const feedbackPolicyVersion = 'feedback-0.3.0';
export function feedbackAdjustment(input: {rpe:number; pain:number; dizziness:boolean; intensityRating?:string;durationRating?:string;frequencyRating?:string;earlyStopped?:boolean}, usedIntensity:number, previousCap:number|null = null, previousHold = false) {
  ...
  const requiresReview = previousHold || input.pain > 0 || input.dizziness || input.durationRating === 'strong' || input.frequencyRating === 'strong';
  const reasonCode = requiresReview ? 'FEEDBACK_HOLD' : reduce ? 'FEEDBACK_INTENSITY_REDUCED' : 'FEEDBACK_MAINTAINED';
  const reason = requiresReview ? '통증·어지럼 또는 시간·주파수 불편 보고가 있어 담당자 확인 전 사용을 보류합니다.' : reduce ? '지난 사용이 힘들었다는 응답을 반영해 강도를 10% 낮췄습니다.' : input.earlyStopped === true ? '사용자 중지는 기록했으며 불편 보고가 없어 다음 사용을 보류하지 않습니다.' : '지난 사용 강도를 유지합니다. 자동으로 높이지 않습니다.';
