...
export function averageMeasurements(
  ...
  if (
    measurements.some((measurement) =>
      averageFields.some((field) => !isFinitePositive(measurement[field])),
    )
  ) {
    warnings.push('필수 측정값에 0, 음수 또는 숫자가 아닌 값이 있습니다.');
  }

  if (measurements.length === 0) {
    return { averagedValues: null, warnings };
  }

  const averagedValues = Object.fromEntries(
    averageFields.map((field) => [
      field,
      round(
        measurements.reduce((sum, measurement) => sum + measurement[field], 0) /
          measurements.length,
      ),
    ]),
  ) as AverageValues;
